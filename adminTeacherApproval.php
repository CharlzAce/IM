...
<html lang="en">
    ...
    <body class="gradientBg">
        <section id="header">
            ...
            <div>
                <ul id="navbar">
                    ...
                    <li><a class="active" href="adminTeacherApproval.php"><i class="fa-solid fa-circle-check"></i>Teacher Approval</a></li>
        ...
        <section>
            <br><br>
            <h2 style="margin-left: 420px;">Teachers Account Approval</h2>
            <div class="container">
                <div class="student-list-container">
                    <div class="header-container">
                        <?php
                            $query = "SELECT COUNT(TEACH_ID) AS NUM_TEACH FROM TEACHER WHERE TEACH_APPROVAL IS NULL;";
                    ...
                    <?php
                        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['teachSearch'])) {
                            ...
                            if($stud_search != null) {
                                $query = "SELECT * FROM TEACHER WHERE TEACH_APPROVAL IS NULL AND TEACH_LNAME = ?;";
                                $stmt = $pdo->prepare($query);
                                            ...
                                            <div class="student-list">
                                                <div class="student-box">
                                                    ...
                                                    <div class="student-actions">
                                                        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                                                            <button class="edit-btn" id="teachEdit" name="teachEdit" value="<?= $row['teach_id'];?>" >Approved</button>
                                                            <button type="button" class="btn delete-btn" data-bs-toggle="modal" data-bs-target="#exampleModal" value="<?= $row['teach_id']; ?>" onclick="setDeleteId(<?= $row['teach_id']; ?>)">Deny</button>
                                                        </form>
                                                    </div>
                                                </div>
                                            </div>
                    ...
                    ?>
                    <?php
                        $query = "SELECT * FROM TEACHER WHERE TEACH_APPROVAL IS NULL;";
                        $stmt = $pdo->prepare($query);
                        $stmt->execute();
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        Are you sure you want to deny this teacher account?
                    </div>
...
<?php
    if ($_SERVER["REQUEST_METHOD"] == "POST" && (isset($_POST['teachEdit']) || isset($_POST['teachDelete']))) {
        ...
        if($teach_edit != null) {
            ...
            $query = "UPDATE TEACHER SET TEACH_APPROVAL = 'Approved' WHERE TEACH_ID = ?;";
            $stmt = $pdo->prepare($query);
            $stmt->execute([$teach_edit]);
    
            echo "\<script type='text/javascript'> ";
            echo "window.location.href='adminTeacherApproval.php';";  
            echo "</script>";
        }
