<?php
    ...
    function authenticateStudent($email, $password, $pdo) {
        $stmt = $pdo->prepare("SELECT * FROM STUDENT WHERE STUD_GRADE_LEVEL = 11 AND STUD_STATUS = 'New' AND STUD_EMAIL = ?");
        $stmt->execute([$email]);
        $admin = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($admin && $password == $admin['stud_password']) {
            return true; // Authentication successful
        } 
        else {
            return false; // Authentication failed
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!empty($email) && !empty($password)) {
            if($email == 'admin@mansci.com') {
                ...
                if (authenticateStudent($email, $password, $pdo)) {
                    // Redirect to New Student Dashboard
                    $_SESSION['email'] = $email;
                    header("Location: student11Dashboard.php");
                    exit();
                } 
                else
                    $error_message = "Invalid Username or Password";
...
<html lang="en">
    <head>
        ...
        <title>ManSci | Grade 11 Sign In</title>
    </head>
                                    ...
                                    <div class="card-body p-md-5 mx-md-4">
                                        ...
                                        <form id="cusLog" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                                            ...
                                            <div class="d-flex align-items-center justify-content-center pb-4">
                                                ...
                                                <button type="button" data-mdb-button-init data-mdb-ripple-init class="btn gradient-custom-2 btn" onclick="window.location.href='student11SignUp.php'"><b>Sign Up</b></button>
                                            </div>
