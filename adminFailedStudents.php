        ...
        <section id="header">
            ...
            <div>
                <ul id="navbar">
                    ...
                    <li><a class="active" href="adminFailedStudents.php"><i class="fa-solid fa-user"></i>Failed Enrollments</a></li>
            ...
            <div class="student-list-container">
                ...
                <?php
                    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['studSearch'])) {
                        ...
                        if($stud_search != null) {
                            $query = "SELECT * FROM STUDENT, ENROLLMENT WHERE STUDENT.STUD_ID = ENROLLMENT.STUD_ID AND EN_APPROVAL = 'Failed' AND STUD_LNAME = ? AND STUD_GRADE_LEVEL = ?;";
                ...
                <?php
                    if($gr != null) {
                        $query = "SELECT * FROM STUDENT, ENROLLMENT WHERE ENROLLMENT.STUD_ID = STUDENT.STUD_ID AND EN_APPROVAL = 'Failed' AND STUD_GRADE_LEVEL = ?;";
                        $stmt = $pdo->prepare($query);
                        $stmt->execute([$gr]);
                                ...
                                ?>
                                    <div class="student-list">
                                        <div class="student-box">
                                            ...
                                            <div class="student-actions">
                                                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                                                    <button class="edit-btn" id="studEdit" name="studEdit" value="<?= $row['stud_id'];?>" >Reset</button>
                                                    <button type="button" class="btn delete-btn" data-bs-toggle="modal" data-bs-target="#exampleModal" value="<?= $row['stud_id']; ?>" onclick="setDeleteId(<?= $row['stud_id']; ?>)">Delete</button>
                                                </form>
                                            </div>
                                        </div>
...
<?php
    if ($_SERVER["REQUEST_METHOD"] == "POST" && (isset($_POST['studEdit']) || isset($_POST['studDelete']))) {
        ...
        if($stud_edit != null) {
            $query = "UPDATE ENROLLMENT SET EN_APPROVAL = 'In Progress' WHERE STUD_ID = ?;";
            $stmt = $pdo->prepare($query);
            $stmt->execute([$stud_edit]);
    
            echo "\<script type='text/javascript'> ";
            echo "window.location.href='adminFailedStudents.php';";  
            echo "</script>";
        }
