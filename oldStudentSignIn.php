<?php
    ...
    function authenticateOldStudent($email, $password, $pdo) {
        $stmt = $pdo->prepare("SELECT * FROM STUDENT WHERE STUD_STATUS = 'Old' AND STUD_EMAIL = ?");
        $stmt->execute([$email]);
        $admin = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!empty($email) && !empty($password)) {
            if($email == 'admin@mansci.com') {
                ...
                if (authenticateOldStudent($email, $password, $pdo)) {
                    // Redirect to New Student Dashboard
                    $_SESSION['email'] = $email;
                    header("Location: oldStudentDashboard.php");
                    exit();
                } 
                else
                    $error_message = "Invalid Username or Password";
...
<html lang="en">
    <head>
        ...
        <title>ManSci | Old Student Sign In</title>
    </head>
